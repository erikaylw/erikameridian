...
export async function getTopCandidates({ limit = 10 } = {}) {
  ...
  const eligible = pools
    .filter((p) => {
      ...
      const maxMcapTvlRatio = config.screening.maxMcapToTvlRatio;
      if (maxMcapTvlRatio != null && p.mcap != null && p.active_tvl > 0) {
        const ratio = p.mcap / p.active_tvl;
        if (ratio > maxMcapTvlRatio) {
          log("screening", `Filtered high mcap/tvl ${p.name}: ratio ${ratio.toFixed(1)}x (limit ${maxMcapTvlRatio}x, mcap=${p.mcap}, tvl=${p.active_tvl})`);
          pushFilteredReason(filteredOut, p, `mcap/tvl ${ratio.toFixed(1)}x exceeds ${maxMcapTvlRatio}x limit`);
          return false;
        }
      }
      return true;
